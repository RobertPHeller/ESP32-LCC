...
"""
    ...
    Components are sorted by ref and grouped by partnum
    Fields are (if exist)
    Item, Qty, Reference(s), Value, LibPart, Footprint, Datasheet

    Command line:
    python "pathToFile/bom_csv_grouped_by_partnum.py" "%I" "%O.csv"
"""
...
def myEqu(self, other):
    ...
    on their partnum and footprint.

    In this example of a custom equivalency operator we compare the
    partnum, the part name and the footprint.
    """
    result = True
    if self.getField("Mouser Part Number") != other.getField("Mouser Part Number"):
        result = False
...
for group in grouped:
    ...
    for component in group:
        if notatrealpart(component.getRef()):
            pass
        else:
            if len(refs) > 0:
                refs += ", "
