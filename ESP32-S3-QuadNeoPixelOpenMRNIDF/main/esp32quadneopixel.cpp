...
esp32quadneopixel::ConfigDef cfg(0);
...
void app_main()
{
    ...
    else
    {
        ...
        LOG(INFO, "[esp32quadneopixel] about to start the Simple Can Stack");
        openlcb::SimpleCanStack stack(nvs.node_id());
        LOG(INFO, "[esp32quadneopixel] stack started");
        stack.set_tx_activity_led(LED_ACT1_Pin::instance());
        LOG(INFO, "[esp32quadneopixel] set activity led");
        ...
        LOG(INFO, "[esp32quadneopixel] MemoryConfigClient done.");
#ifdef CONFIG_ESP32_WIFI_ENABLED
