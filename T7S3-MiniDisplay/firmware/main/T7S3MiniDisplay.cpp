...
void app_main()
{
    ...
    LOG(INFO, "[T7S3MiniDisplay] about to start the PhysicalTrainNode");
    ...
    LOG(INFO, "[T7S3MiniDisplay] MemoryConfigClient done.");
    T7S3MiniDisplay::FactoryResetHelper factory_reset_helper;
    LOG(INFO, "[T7S3MiniDisplay] FactoryResetHelper done.");
    reboothelpers::DelayRebootHelper delayed_reboot(stack.service());
    LOG(INFO, "[T7S3MiniDisplay] DelayRebootHelper done.");
    healthmonitor::HealthMonitor health_mon(stack.service());
    LOG(INFO, "[T7S3MiniDisplay] HealthMonitor done.");
    WiFiManager wifi_manager(nvs.station_ssid(),
                             nvs.station_pass(),
                             &stack, 
