...
WiFiManager::WiFiManager(const char *station_ssid
...
void WiFiManager::display_configuration()
{
...
ConfigUpdateListener::UpdateAction 
      WiFiManager::apply_configuration(int fd, bool initial_load,
                                       BarrierNotifiable *done)
{
    AutoNotify n(done);
    esp32Params_.reset(new Esp32SocketParams(fd,cfg_));
    if (!enabled_) return ConfigUpdateListener::UPDATED;
    if (initial_load)
    {
        //espIdfWiFi_.disable_mdns_publish_on_sta ();
        espIdfWiFi_.init();
        espIdfWiFi_.set_ip_acquired_callback(std::bind(&WiFiManager::connect_callback_,
                                                       this,
                                                       std::placeholders::_1,
                                                       std::placeholders::_2));
...
void WiFiManager::factory_reset(int fd)
{
...
void WiFiManager::connect_callback_(WiFiDefs :: Interface iface , 
                                                          bool acquired )
{
    if (acquired && ! mdns_started_)
    {
        mdns_started_ = true;
        LOG(INFO,"[WiFiManager::WiFiStackFlow] connect_callback_() ip acquired");
        uplink_.reset(new SocketClient(stack_->service(), stack_->executor(),
                                       stack_->executor(), 
                                       std::move(esp32Params_),
                                       std::bind(&WiFiManager::on_uplink_created, 
                                                 this,
                                                 std::placeholders::_1, 
                                                 std::placeholders::_2)));
...
void WiFiManager::on_uplink_created(int fd, Notifiable *on_exit)
{
