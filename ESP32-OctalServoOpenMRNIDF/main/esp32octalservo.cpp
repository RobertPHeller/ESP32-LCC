...
{
    Singleton<esp32octalservo::NodeRebootHelper>::instance()->reboot();
    return nullptr;
}
...
void app_main()
{
    ...
    esp32octalservo::NvsManager nvs;
    nvs.init(reset_reason);
    
    LOG(INFO, "[BootPauseHelper] starting...");
    
    esp32octalservo::BootPauseHelper pause;
    ...
    openmrn_arduino::Esp32WiFiManager wifi_manager(
                                                   ...
                                                   nvs.hostname_prefix());
#endif
    esp32octalservo::FactoryResetHelper factory_reset_helper();
    LOG(INFO, "[MAIN] FactoryResetHelper allocated");
    esp32octalservo::EventBroadcastHelper event_helper();
    LOG(INFO, "[MAIN] EventBroadcastHelper allocated");
    esp32octalservo::HealthMonitor health_mon(stack.service());
    ...
    stack.check_version_and_factory_reset(cfg.seg().internal_config(),
                                          CDI_VERSION,
                                          cleanup_config_tree);
    esp32octalservo::NodeRebootHelper node_reboot_helper(&stack, config_fd);
